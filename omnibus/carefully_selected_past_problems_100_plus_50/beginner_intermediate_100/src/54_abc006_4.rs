    ...
    W: Write,
{
...

    {
        ...
        // dp[increasing_length] = min_last_card
        let mut dp = nested_vec![None; n + 1];
        dp[0] = 0.into();
        dp[1] = c[0].into();
        for i in 1..n {
            let border = dp.search_true_false_border(|&x| x.map_or(false, |x| x < c[i]));
            // O(logN)
            dp[border.max_true.unwrap() + 1] = c[i].into();
        } // O(NlogN)

        println!(
            "{}",
            n - dp.iter().enumerate().max_by_key(|x| x.1).unwrap().0
        ) // O(N);
    } // O(NlogN)
