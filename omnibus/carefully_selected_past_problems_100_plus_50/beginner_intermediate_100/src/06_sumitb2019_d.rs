    ...
    W: Write,
{
...

    {
        ...
        let mut possible_pins = HashSet::new();

        for s_i in s.iter() {
            let s_i = s_i.to_digit(10).unwrap();

            for &num in twos.iter() {
                possible_pins.insert(num * 10 + s_i);
            } // O(1)  ( O(10) )

            for &digits in ones.iter() {
                twos.insert(digits * 10 + s_i);
            } // O(1)  ( O(100) )

            ones.insert(s_i);
        } // O(N)

        println!("{}", possible_pins.len());
    } // O(N)
