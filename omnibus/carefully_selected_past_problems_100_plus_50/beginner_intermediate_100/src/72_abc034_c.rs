    ...
    W: Write,
{
    ...
    {
        use modular::CombinationGenerator;
        modulo!(1_000_000_007 as Mod1Billion7);

        input! {
            w: Length, h: Length,
        }

        // move up (h - 1) times in route_length (w - 1 + h - 1)
        let result = CombinationGenerator::<Mod1Billion7>::new(w + h)
            .generate(w + h - 2, h - 1)
            .value();
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "4 3", "10");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "123 456", "210368064");
    }
}
