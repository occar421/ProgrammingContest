...

    {
        ...
        impl Tree {
            ...
            pub fn insert(&mut self, position: usize) {
                let none_node = Self::find_node_to_insert(&mut self.root, position);

                *none_node = TreeNode::Node {
                    position,
                    left: TreeNode::None.into(),
                    right: TreeNode::None.into(),
                };
            }

            fn find_node_to_insert(node: &mut TreeNode, position: usize) -> &mut TreeNode {
                match node {
                    TreeNode::None => node,
                    TreeNode::Node {
                        position: node_pos,
                        ...
                        if position <= *node_pos {
                            Self::find_node_to_insert(left, position)
                        } else {
                            Self::find_node_to_insert(right, position)
                        }
                    }
                }
            }
            ...
            /// O( log(n) )
            pub fn calc_length(&self, position: usize) -> Length {
                let normal_try = self.calc_length_inner(&self.root, position, 0, self.ring_length);
                let extra_try = self.calc_length_inner(
                    &self.root,
                    position + self.ring_length,
                    self.ring_length,
                    self.ring_length * 2,
                );
                normal_try.min(extra_try)
            }

            fn calc_length_inner(
                &self,
                node: &TreeNode,
                position: usize,
                left_pos: usize,
                ...
                match node {
                    TreeNode::None => (position - left_pos).min(right_pos - position),
                    TreeNode::Node {
                        position: node_pos, ..
                    } if position == *node_pos => 0,
                    TreeNode::Node {
                        position: node_pos,
                        left,
                        right,
                        ...
                        if position <= *node_pos {
                            self.calc_length_inner(left, position, left_pos, *node_pos)
                        } else {
                            self.calc_length_inner(right, position, *node_pos, right_pos)
                        }
                    }
                }
            }
        ...
        let result: usize = k.iter().map(|&k_m| tree.calc_length(k_m)).sum(); // O( m log(n) )
