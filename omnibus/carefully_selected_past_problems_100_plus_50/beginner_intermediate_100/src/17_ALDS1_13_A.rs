    ...
    W: Write,
{
    ...
    fn put_queen(controlled_board_flags: &mut [[bool; SIZE]; SIZE], row: usize, column: usize) {
        // vertical
        for i in 0..SIZE {
            controlled_board_flags[i][column] = true;
        }
        // horizontal
        for j in 0..SIZE {
            controlled_board_flags[row][j] = true;
        }
        ...
        if row >= column {
            // bottom-left part
            let skew = row - column;
            for i in 0..(SIZE - skew) {
                controlled_board_flags[i + skew][i] = true;
            }
        } else {
            // top-right part
            let skew = column - row;
            for i in 0..(SIZE - skew) {
                controlled_board_flags[i][i + skew] = true;
            }
        }

        // bottom-left to top-right
        if SIZE - 1 - row >= column {
            // top-left part
            let skew = SIZE - 1 - row - column;
            for i in 0..(SIZE - skew) {
                controlled_board_flags[SIZE - 1 - (i + skew)][i] = true;
            }
        } else {
            // bottom-right part
            let skew = column - (SIZE - 1 - row);
            for i in 0..(SIZE - skew) {
                controlled_board_flags[SIZE - 1 - i][i + skew] = true;
            }
        }
    }

    let mut controlled_board_flags = [[false; SIZE]; SIZE];

    for &(row, column) in rc.iter() {
        put_queen(&mut controlled_board_flags, row, column)
    }

    let all_rows: HashSet<_> = (0..SIZE).collect();

    fn find_dfs(
        all_rows: &HashSet<usize>,
        positions: &mut Vec<(usize, usize)>,
        controlled_board_flags: &mut [[bool; SIZE]; SIZE],
    ) -> Option<Vec<(usize, usize)>> {
        let rows: HashSet<_> = positions.iter().map(|&(row, _)| row).collect();
        let row = all_rows.difference(&rows).into_iter().min();
        return if let Some(&row) = row {
            for j in 0..SIZE {
                if !controlled_board_flags[row][j] {
                    let mut controlled_board_flags = controlled_board_flags.clone();
                    put_queen(&mut controlled_board_flags, row, j);
                    positions.push((row, j));
                    let result = find_dfs(all_rows, positions, &mut controlled_board_flags);
                    positions.pop();
                    if result.is_some() {
                        return result;
                    }
    ...
    let mut positions = rc.clone();
    let mut positions = find_dfs(&all_rows, &mut positions, &mut controlled_board_flags).unwrap();

    positions.sort_by(|&(row1, _), &(row2, _)| row1.cmp(&row2));

    for i in 0..SIZE {
        let (_, column) = positions[i];
        for _ in 0..column {
            write!(writer, ".")?;
        }
        write!(writer, "Q")?;
        for _ in column + 1..SIZE {
            write!(writer, ".")?;
        }
        writeln!(writer)?;
    }
