...

    {
        ...
        impl Ord for Node {
            fn cmp(&self, other: &Self) -> Ordering {
                // less_cost, more_moves
                self.cost
                    .cmp(&other.cost)
                    .reverse()
                    .then(self.remaining_moves.cmp(&other.remaining_moves))
            }
        }
        ...
        let mut unused_nodes = vec![true; n];
        while let Some(node) = heap.pop() {
            ...
            if node.remaining_moves > 0 {
                for &next in edges[&node.id].iter() {
                    heap.push(Node {
                        id: next,
                        cost: node.cost,
                        remaining_moves: node.remaining_moves - 1,
                    })
                }
            }

            if unused_nodes[node.id] {
                heap.push(Node {
                    id: node.id,
                    cost: node.cost + cr[node.id].0,
                    remaining_moves: cr[node.id].1,
                });
                unused_nodes[node.id] = false;
            }
        } // O((N+K)logN)
    } // O((N+K)logN)
