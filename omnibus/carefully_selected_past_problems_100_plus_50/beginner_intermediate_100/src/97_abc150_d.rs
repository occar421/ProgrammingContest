    ...
    W: std::io::Write,
{
...

    {
        input! {
            n: usize, m: usize,
            a: [usize; n],
        } // below O(NlogN), O(logM)

        let mul2 = a[0].trailing_zeros();
        // O(N)
        if a.iter().any(|&a_i| a_i.trailing_zeros() != mul2) {
            println!("0");
        } else {
            let lcm_of_a = a.iter().fold(1usize, |acc, &a_i| lcm(acc, a_i)) / 2; // O(NlogN)

            println!("{}", m / lcm_of_a - m / (lcm_of_a * 2));
        }
    } // O(NlogN)
