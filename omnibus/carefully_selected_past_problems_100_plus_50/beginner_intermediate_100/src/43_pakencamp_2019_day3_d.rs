    ...
    W: Write,
{
...

    {
        ...
        let mut remains = Vec::with_capacity(n);
        for j in 0..n {
            ...
            for i in 0..5 {
                if s[i][j] != '#' {
                    *buckets.get_mut(&s[i][j]).unwrap() -= 1;
                }
            } // O(1)
            remains.push(buckets);
        } // O(N)

        let mut edges = HashMap::new();
        edges.insert((0, 'S'), vec![]);
        for c in colors.iter() {
            edges
                .get_mut(&(0, 'S'))
                .unwrap()
                .push((remains[0][c], (1, *c)));
        } // O(1)
        for j in 1..n {
            for c in colors.iter() {
                edges.insert(
        ...
        heap.push(Reverse((0, (0, 'S'))));

        let mut result = 0;
        while let Some(Reverse((cost, node))) = heap.pop() {
            if reached_nodes.contains_key(&node) {
                continue;
            }
            reached_nodes.insert(node, cost);
            ...
            for (next_cost, next_node) in edges[&node].iter() {
                heap.push(Reverse((cost + next_cost, *next_node)))
            }
        } // O(NlogN) : O((E+V)logV), V=N, E=N
