    ...
    W: Write,
{
...

    {
        ...
        fn search_dfs(coins: &[usize], n: usize) -> Quantity {
            fn inner(
                n_coins: usize,
                min_n_coins_record: usize,
                possible_coins: &[usize],
                rest: usize,
            ) -> Option<Quantity> {
                if min_n_coins_record <= n_coins {
                    return None;
                }
                let &target_coin = possible_coins.first()?;

                if target_coin > rest {
                    None
                } else if rest % target_coin == 0 {
                    Some(n_coins + rest / target_coin)
                } else if n_coins + rest / target_coin /* = best */ > min_n_coins_record {
                    None
                } else {
                    let mut min_n = min_n_coins_record;
                    for i in 0..possible_coins.len() {
                        if let Some(result) =
                            inner(n_coins + 1, min_n, &possible_coins[i..], rest - target_coin)
                        {
                            min_n = min_n.min(result);
                        }
                    }
                    Some(min_n)
                }
            ...
            let mut min_n = std::usize::MAX;
            for i in 0..coins.len() {
                if let Some(result) = inner(0, min_n, &coins[i..], n) {
                    min_n = min_n.min(result);
                }
            }
            return min_n;
