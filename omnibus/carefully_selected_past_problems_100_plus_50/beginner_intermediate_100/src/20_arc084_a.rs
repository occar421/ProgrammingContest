    ...
    W: Write,
{
...

    {
        ...
        for b_i in b.iter() {
            let border = c.search_false_true_border(|c_j| c_j > b_i);
            bcs.push(n - border.min_true.unwrap_or(n));
        } // O(NlogN)
        let bcs_cum_sum = cumulative_sum::CumulativeSum1d::new(n, &bcs);
        // O(N)

        let mut abcs = Vec::with_capacity(n);
        for a_i in a.iter() {
            let border = b.search_false_true_border(|b_j| b_j > a_i);
            if let Some(min_true) = border.min_true {
                abcs.push(bcs_cum_sum.sum_in(min_true..));
            } else {
                // never comes Some after here
                break;
            }
        } // O(NlogN)

        println!("{}", abcs.iter().sum::<usize>());
    } // O(NlogN)
