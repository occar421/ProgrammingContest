    ...
    W: std::io::Write,
{
...

    {
        loop {
            ...
            dbg!(pieces);
            for &(p, s) in ps.iter() {
                let (w, d) = pieces.remove(p);
                let s = s % (w + d + w + d);
                if s <= w {
                    let smaller = min!(w - s, s);
                    pieces.push((smaller, d));
                    pieces.push((w - smaller, d));
                } else if (s - w) <= d {
                    let s = s - w;
                    let smaller = min!(d - s, s);
                    pieces.push((w, smaller));
                    pieces.push((w, d - smaller));
                } else if (s - w - d) <= w {
                    let s = s - w - d;
                    let smaller = min!(w - s, s);
                    pieces.push((smaller, d));
            ...
            println!("{}", pieces.into_iter().map(|(w, d)| w * d).easy_join(" "));
        }
    }
