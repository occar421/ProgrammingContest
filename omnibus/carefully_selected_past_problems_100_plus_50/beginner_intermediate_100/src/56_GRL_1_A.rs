    ...
    W: std::io::Write,
{
...

    {
        ...
        for (s, t, d) in std {
            edges.entry(s).or_insert(vec![]).push((t, d));
        }

        let mut heap = BinaryHeap::new();
        let mut visited_nodes = vec![None; v];

        heap.push(Reverse((0usize, r)));
        while let Some(Reverse((cost, current))) = heap.pop() {
            if visited_nodes[current].is_some() {
                continue;
            }
            visited_nodes[current] = cost.into();

            if let Some(edges) = edges.get(&current) {
                for &(dest, move_cost) in edges.iter() {
                    heap.push(Reverse((cost + move_cost, dest)));
                }
            }
        }
