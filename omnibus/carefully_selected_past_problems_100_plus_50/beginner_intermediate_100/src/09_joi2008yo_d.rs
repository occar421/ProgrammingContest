    ...
    W: Write,
{
...

    {
        ...
        if m == 1 {
            println!(
                "{} {}",
                points[0].0 - sign_points[0].0,
                points[0].1 - sign_points[0].1
            );
            return Ok(());
        }

        let mut relative_sign_points = Vec::with_capacity(m - 1);
        for i in 1..m {
            let diff_x = sign_points[i].0 - sign_points[0].0;
            let diff_y = sign_points[i].1 - sign_points[0].1;
            relative_sign_points.push((diff_x, diff_y));
        } // O(M)

        let point_set: HashSet<_> = HashSet::from_iter(points.iter()); // O(N)
        for i in 0..n {
            let axis = points[i];
            if relative_sign_points
                .iter()
                .all(|a| point_set.contains(&(axis.0 + a.0, axis.1 + a.1)))
            // O(M)
            {
                println!(
                    "{} {}",
                    axis.0 - sign_points[0].0,
                    axis.1 - sign_points[0].1
                );
                break;
            }
        } // O(NM)
