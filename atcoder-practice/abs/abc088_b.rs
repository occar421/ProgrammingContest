    ...
    W: Write,
{
    ...
    let iter = a.chunks_exact(2);
    let last = *iter.remainder().get(0).unwrap_or(&0);
    let scores = iter.fold((last, 0), |acc, pair| (acc.0 + pair[0], acc.1 + pair[1]));

    writeln!(writer, "{}", scores.0 - scores.1)?;
