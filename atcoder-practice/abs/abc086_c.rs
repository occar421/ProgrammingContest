    ...
    W: Write,
{
    ...
    let plans = {
        let mut plans = vec![(0, 0, 0)];
        plans.append(&mut txy);
        plans
    };

    for movement in plans.windows(2) {
        if let [from, to] = movement {
            let time = to.0 - from.0;
            let manhattan_distance = (from.1 - to.1).abs() + (from.2 - to.2).abs();

            if manhattan_distance > time || (manhattan_distance - time) % 2 != 0 {
                writeln!(writer, "No")?;
                return Ok(());
            }
        }
