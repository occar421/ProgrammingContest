    ...
    W: std::io::Write,
{
...

    {
        ...
        for i in 0..n {
            ...
            for j in 0..n {
                let v = a[i] ^ b[j];
                map.entry(v).or_insert(vec![]).push(j);
            } // O(N)
            values.push(map);
        } // O(N^2)
        ...
        struct DfsEnv<'a> {
            n: usize,
            values: &'a Vec<HashMap<usize, Vec<usize>>>,
            used_indices: &'a mut HashSet<usize>,
        }
        fn dfs(
            DfsEnv {
                n,
            ...
            if index == *n {
                true
            } else if let Some(vx) = values[index].get(&target) {
                ...
                for &v in vx.iter() {
                    ...
                    result |= dfs(&mut env, index + 1, target);
        ...
        'outer: for (&k, vx) in values[0].iter() {
            ...
            for &v in vx.iter() {
                ...
                let mut env = DfsEnv {
                    n,
                    values: &values,
                    used_indices: &mut used_indices,
                };
                let is_good = dfs(&mut env, 1, k);
                used_indices.remove(&v);
                if is_good {
                    good_numbers.push(k);
                    continue 'outer;
        ...
        println!("{}", good_numbers.len());
        for &good_number in good_numbers.iter() {
            println!("{}", good_number);
        }
    }
