    ...
    W: std::io::Write,
{
...

    {
        ...
        modulo!(998_244_353 as ModSub1M);
        // modulo!(1_000_000_007 as ModSub1M);
        ...
        let cum_sum = cumulative_sum::CumulativeSum1d::new(n, &possible);
        let mut hoge = cum_sum.dump()[1..].to_vec();
        for &(_, k) in ck.iter() {
            hoge[k] = 1;
        }
        dbg!(hoge);

        let result: ModSub1M = hoge.iter().map(|x| ModSub1M::new(*x as usize)).product();
