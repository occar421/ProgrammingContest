    ...
    W: std::io::Write,
{
...

    {
        ...
        let mut a_to_b = vec![usize::max_value(); n];
        let mut first_seen = HashMap::new();
        for (i, &a_i) in a.iter().enumerate() {
            let point = min!(i, *first_seen.get(&a_i).unwrap_or(&i));
            for (j, &b_j) in b.iter().enumerate().skip(point) {
                dbg!(i, a_i, j, b_j);
                if a_i == b_j {
                    a_to_b[i] = j;
                    break;
                }
                if first_seen.contains_key(&b_j) {
                    continue;
                }
                first_seen.insert(b_j, j);
            }
