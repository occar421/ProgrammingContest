    ...
    W: std::io::Write,
{
...

    {
        ...
        let s_set: HashSet<_> = HashSet::from_iter(s.iter());
        let t_set: HashSet<_> = HashSet::from_iter(t.iter());

        if !s_set.is_superset(&t_set) {
            println!("-1");
            return Ok(());
        }
        ...
        if t_set.len() == 1 {
            let nearest = min!(
                s.iter().position(|x| *x == t[0]).unwrap(),
                n - s.iter().rposition(|x| *x == t[0]).unwrap()
            );
            println!("{}", t.len() + nearest);
            return Ok(());
        }

        let mut count = 0;
        if t[0] == s[0] {
            count += t.iter().take_while(|&x| *x == t[0]).count();
        }
        let i = count;
        let nearest = min!(
            s.iter().position(|x| *x == t[count]).unwrap(),
            n - s.iter().rposition(|x| *x == t[count]).unwrap()
        );
        dbg!(nearest, count);
        count += nearest;
        let mut prev_t_i = t[i];
        for i in i..t.len() {
            dbg!(i, prev_t_i);
            if t[i] == prev_t_i {
                count += 1; // append
            } else {
                count += 2; // dial and append
            }
            prev_t_i = t[i];
            dbg!(count);
            dbg!();
