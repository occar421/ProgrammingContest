    ...
    W: Write,
{
...

    {
        ...
        d.push(0); // head store
        d.sort(); // O( n log(n) )
        d.push(l); // head store as l

        fn binary_search(d: &Vec<usize>, left: usize, right: usize, position: usize) -> usize {
            let middle = left + (right - left) / 2;
            if left == middle {
                return (position - d[left]).min(d[right] - position);
            }
            match position.cmp(&d[middle]) {
                Ordering::Less => binary_search(d, left, middle, position),
                Ordering::Equal => 0,
                Ordering::Greater => binary_search(d, middle, right, position),
            }
        } // O( log(n) )

        let result: usize = k
            .iter()
            .map(|&k_m| binary_search(&d, 0, d.len(), k_m))
            .sum(); // O( m log(n) )
