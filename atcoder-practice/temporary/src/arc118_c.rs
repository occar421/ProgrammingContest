...
fn get_primes(n: usize) -> HashSet<usize> {
    let max = (n as f64).exp().ceil() as usize;
    ...
    W: Write,
{
    ...
    let mut primes: Vec<_> = primes.iter().collect();
    primes.sort();
    let primes: Vec<_> = primes.iter().take(n).collect();
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "4", "84 60 105 70");
    }
}
