    ...
    W: Write,
{
...

    {
        ...
        if s.iter().filter(|&&s_i| s_i == '0').count()
            != t.iter().filter(|&&t_i| t_i == '0').count()
        {
            println!("-1");
            return Ok(());
        }
        ...
        for i in 1..s.len() {
            if s[i] != current {
                changes.insert(i, s[i]);
                current = s[i];
            }
        }

        dbg!(s.iter().easy_join(""), t.iter().easy_join(""));
        dbg!(&changes);

        let mut counter = 0;
        for i in 0..n {
            if s[i] != t[i] {
                let (j, &sj) = changes.iter().next().unwrap();
                let mut j = *j;
                changes.remove(&j);
                if sj != t[i] {
                    j = *changes.iter().next().unwrap().0;
                    changes.remove(&j);
                }
                dbg!(i, s[i], j, s[j]);
                swap!(s[i], s[j]);
                ...
                let (&k, &sk) = changes.iter().next().unwrap();
                dbg!(j + 1 == k, i + 1 == j);
                if j + 1 == k {
                    changes.remove(&k);
                    if i + 1 == j {
                        changes.insert(j, s[j]);
                    } else {
                        changes.insert(i + 1, s[j]);
                    }
                } else {
                    changes.insert(i + 1, s[i + 1]);
                    changes.insert(j + 1, s[j + 1]);
                    changes.insert(k + 1, s[k + 1]);
                }
