    ...
    W: Write,
{
    ...
    let i_diffs: Vec<_> = (0..n).map(|i| c[i][0] - c[0][0]).collect();
    let j_diffs: Vec<_> = (0..n).map(|j| c[0][j] - c[0][0]).collect();

    let mut is_ok = true;
    'outer: for i in 1..n {
        for j in 1..n {
            if !(c[i][j] - c[0][j] == i_diffs[i] && c[i][j] - c[i][0] == j_diffs[j]) {
                is_ok = false;
                break 'outer;
            }
        }
    ...
    if is_ok {
        let i_min = i_diffs.iter().min().unwrap();
        let j_min = j_diffs.iter().min().unwrap();
        let jack_up_number = if i_min <= j_min {
            (i_min.abs(), c[0][0] - i_min.abs())
        } else {
            (c[0][0] - j_min.abs(), j_min.abs())
        };

        write!(writer, "Yes\n")?;
        write!(writer, "{}\n", i_diffs.iter().map(|x| x + jack_up_number.0).easy_join(" "));
        write!(writer, "{}", j_diffs.iter().map(|x| x + jack_up_number.1).easy_join(" "));
    } else {
        write!(writer, "No")?;
    }
