    ...
    W: Write,
{
    ...
    let mut ak_max = 0;
    let mut prev_ak_sum = 0;
    let mut prev_ak_acc = 0;
    for (k, &ak) in a.iter().enumerate() {
        let k = k + 1;
        ak_max = ak_max.max(ak);
        let ak_sum = prev_ak_sum + ak; // prev - (prev max*k)
        let ak_acc = prev_ak_acc + ak_sum; // sigma k

        writeln!(writer, "{}", ak_acc + ak_max * k);
