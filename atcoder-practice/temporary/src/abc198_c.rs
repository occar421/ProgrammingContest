    ...
    W: Write,
{
    ...
    if distance_square == r * r {
        write!(writer, "1")?;
        return Ok(());
    }
    if distance_square < 2 * r * r {
        write!(writer, "2")?;
        return Ok(());
    }
    ...
    writeln!(
        writer,
        "{}",
        (distance_square as f64 / (r * r) as f64).sqrt().ceil()
    )?;
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "5 15 0", "3");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "5 11 0", "3");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "3 4 4", "2");
    }
}
