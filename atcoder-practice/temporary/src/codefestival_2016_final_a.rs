    ...
    W: Write,
{
    input! {
        stdin = reader;
        h: Quantity, w: Quantity,
        s: [[String; w]; h]
    }

    for (row_index, row) in s.iter().enumerate() {
        if let Some(column_index) = row.iter().position(|item| item == "snuke") {
            writeln!(
                writer,
                "{}{}",
                (column_index + 'A' as usize) as u8 as char,
                row_index + 1
            )?;
            return Ok(());
        }
