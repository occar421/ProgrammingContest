    ...
    W: Write,
{
...

    {
        ...
        let result: usize = b
            .iter()
            .map(|b_i| {
                let a_border = a.search_true_false_border(|a_j| a_j < b_i);
                let c_border = c.search_false_true_border(|c_j| b_i < c_j);
                // O(logN)
                (a_border.max_true.map_or(0, |a_t| a_t + 1))
                    * (c_border.min_true.map_or(0, |c_t| n - c_t))
            })
            .sum(); // O(NlogN)
