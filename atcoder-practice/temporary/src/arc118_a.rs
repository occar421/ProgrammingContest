    ...
    W: Write,
{
    ...
    let mut missing_flags = vec![true; period];
    for i in 0..100 {
        let tax_included_price = (100 + t) * i / 100;
        missing_flags[tax_included_price] = false;
    }

    let missing_numbers: Vec<_> = missing_flags
        .iter()
        .enumerate()
        .filter_map(|(i, &flag)| flag.then_some_(i))
        .collect();

    let index = (n - 1) % missing_numbers.len();
    let multiple = (n - 1) / missing_numbers.len();

    writeln!(writer, "{}", multiple * period + missing_numbers[index])?;
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "10 1", "10");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "3 5", "171");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "1 1000000000", "100999999999");
    }
}
