    ...
    W: std::io::Write,
{
...

    {
        ...
        for (i, p_i) in p.into_iter().enumerate() {
            let i = i + 1;
            v -= 8 - p_i;
            initial_state += i * 10usize.pow(8 - p_i as u32);
        }

        let mut q = VecDeque::new();
        q.push_back((initial_state, 8 - v, 0));
        let mut seen = HashSet::new();
        while let Some((state, hole, cost)) = q.pop_front() {
            ...
            for &next in edges[hole].iter() {
                let current_digit = 10usize.pow(8 - hole as u32);
                let next_digit = 10usize.pow(8 - next as u32);

                let next_piece = state / next_digit % 10;
                let mut next_state = state;
                next_state += next_piece * current_digit;
                next_state -= next_piece * next_digit;

                q.push_back((next_state, next, cost + 1));
            }
        }
