...

    {
        ...
        for i in 0..h {
            for j in 0..w {
                ...
                for p in adjacent2d_4neighbors((i, j), ..h, ..w) {
                    if s[p.0][p.1] == '.' {
                        edges.push((p, 0usize));
                    }
                }
                for p in Adjacent2d::new((i, j), ..h, ..w, punch_area.iter()) {
                    edges.push((p, 1usize));
                }
            }
        }
        ...
        let result = graph.x01bfs((0, 0)).cost_to((h - 1, w - 1)).unwrap();
        println!("{}", result);
    }
