    ...
    W: std::io::Write,
{
...

    {
        ...
        while let Some(((y, x), cost)) = queue.pop_front() {
            ...
            for (dy, dx) in adjacent::adjacent2d_4neighbors((y, x), ..h, ..w) {
                if s[dy][dx] == '.' {
                    queue.push_front(((dy, dx), cost));
                }
            }

            for (dy, dx) in adjacent::Adjacent2d::new((y, x), ..h, ..w, punch_area.iter()) {
                queue.push_back(((dy, dx), cost + 1));
            }
