    ...
    W: std::io::Write,
{
    ...
    // after checking editorial
    {
        ...
        let set_gen = BitBasedSet::generator_of(10);
        for (i, s_i) in s.iter().enumerate().map(|(i, &s_i)| (i + 1, c_to_i(s_i))) {
            dbg_raw!(i, ('A' as u8 + s_i as u8) as char);

            for u in 1..dp[i].len() {
                let u = set_gen.from(u);
                for last in 0..10 {
                    let prev = dp[i - 1][u.dump()][last];
                    if prev > Mod1MilSub::zero() {
                        dbg_raw!(
                            u.into_iter()
                                .map(|x| ('A' as u8 + x as u8) as char)
                                .easy_join(""),
                            ('A' as u8 + last as u8) as char,
                            prev,
                            u.appended_set(s_i)
                                .into_iter()
                                .map(|x| ('A' as u8 + x as u8) as char)
            ...
            dp[i][set_gen.empty_set().appended_set(s_i).dump()][s_i] += 1; // new

            dbg_raw!(dp[i]
                .iter()
                .enumerate()
                .filter(|(_, x)| x.iter().sum::<Mod1MilSub>() > Mod1MilSub::zero())
                .map(|(i, x)| {
                    (
                        set_gen
                            .from(i)
                        ...
                        HashMap::<_, _>::from_iter(
                            x.iter()
                                .enumerate()
                                .filter(|(i, &cnt)| cnt > Mod1MilSub::zero())
                                .map(|(i, &cnt)| (('A' as u8 + i as u8) as char, cnt)),
                        ),
                    )
                })
                .collect::<Vec<_>>());
        ...
        let result: Mod1MilSub = dp
            .last()
            .unwrap()
            .iter()
            .map(|x| x.iter().sum::<Mod1MilSub>())
            .sum();
