    ...
    W: std::io::Write,
{
...

    {
        ...
        input! {
            n: usize,
            mut xy: [(Point2d<isize>); n],
        }

        xy.sort_by_key(|p| p.x);

        let result = (0..=10isize.pow(9)).search_true_false_border(|&k| {
            let mut queue = VecDeque::<Point2d<isize>>::new();
            let mut min_y = 10isize.pow(9) + 1;
            let mut max_y = 0;

            for &right in xy.iter() {
                while let Some(left) = queue.pop_front() {
                    ...
                    min_y = min!(min_y, left.y);
                    max_y = max!(max_y, left.y);
                }

                if min_y <= right.y - k || max_y >= right.y + k {
                    return true;
                }
        ...
        println!("{}", result.max_true.unwrap_or(0));
    }
