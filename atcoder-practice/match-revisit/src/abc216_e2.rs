    ...
    W: std::io::Write,
{
...

    {
        ...
        // len(a[i] >= 0<=m<=10^9]) <= K?
        let result = (0..=(2 * 10usize.pow(9))).search_false_true_border(|&m| {
            a.iter()
                .filter_map(|&a_i| (a_i >= m).then_(|| a_i - m + 1))
                .sum::<usize>()
                <= k
        });

        let m = result.min_true.unwrap();

        if m == 0 {
            let result = a.iter().map(|&a_i| a_i * (a_i + 1) / 2).sum::<usize>();
            println!("{}", result);
            return Ok(());
        }
