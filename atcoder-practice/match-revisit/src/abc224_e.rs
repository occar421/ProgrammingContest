    ...
    W: std::io::Write,
{
...

    {
        input! {
            ...
            rca: [((Point2d<usize1>-rev), usize); n],
        }
        let pa = rca;

        let mut x_maxes: Vec<Option<usize>> = vec![None; w];
        let mut y_maxes: Vec<Option<usize>> = vec![None; h];

        let group = pa.iter().group_with_key(|(_, a)| *a);
        let mut grouped_pa: Vec<_> = group.iter().collect();
        grouped_pa.sort_unstable_by_key(|x| Reverse(x.0));
        let mut result_map = HashMap::new();

        for (_, pa) in grouped_pa {
            dbg!(x_maxes);
            dbg!(y_maxes);

            let mut queue = VecDeque::new();
            for (p, _) in pa.iter() {
                let m = match (x_maxes[p.x], y_maxes[p.y]) {
                    (Some(x), Some(y)) => max!(x, y) + 1,
                    (x, y) => x.or(y).map(|x| x + 1).unwrap_or(0),
                };
                queue.push_back(m);
                result_map.insert(p.clone(), m);
            }

            for (p, _) in pa.iter() {
                let m = queue.pop_front().unwrap();
                x_maxes[p.x] = max!(x_maxes[p.x].unwrap_or(0), m).into();
                y_maxes[p.y] = max!(y_maxes[p.y].unwrap_or(0), m).into();
            }
        }

        for (p, _) in pa {
            println!("{}", result_map[&p]);
        }
    }
