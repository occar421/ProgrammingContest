    ...
    W: std::io::Write,
{
...

    {
        ...
        for (i, &b_i) in b.iter().enumerate() {
            map.entry(b_i).or_insert(VecDeque::new()).push_back(i);
        }
        let mut encoded_a = Vec::with_capacity(n);
        for a_i in a.iter() {
            if let Some(queue) = map.get_mut(a_i) {
        ...
        // inversion number
        let mut bit = binary_indexed_tree::BinaryIndexedTree1d::<usize>::new_with_zeros(n);
        let mut result = 0;
        for &a_i in encoded_a.iter() {
            result += bit.sum_in(a_i..);
            bit.add_value_at(a_i, 1);
        }
