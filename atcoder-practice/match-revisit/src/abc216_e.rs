    ...
    W: std::io::Write,
{
...

    {
        ...
        if sum!(a) < k {
            let all: usize = a.iter().map(|&a_i| (a_i + 1) * a_i / 2).sum();
            println!("{}", all);
            return Ok(());
        }
        ...
        for i in 0..n - 1 {
            let diff = a[i] - a[i + 1];
            let n_affected_arrays = i + 1;
            dbg!(diff * i, rest_k, diff * n_affected_arrays);
            if diff * n_affected_arrays >= rest_k {
                let local_diff = rest_k / n_affected_arrays;
                value += (a[i] + a[i] - local_diff + 1) * local_diff / 2 * n_affected_arrays;
                dbg!(value);
                let remains = rest_k % n_affected_arrays;
                value += remains * (a[i] - local_diff);
                dbg!(value);
                println!("{}", value);
                return Ok(());
            }

            value += (a[i] + a[i] - diff + 1) * diff / 2 * n_affected_arrays;
            rest_k -= diff * n_affected_arrays;
            dbg!(value, rest_k);
        }

        let base_a = a[n - 1];

        let local_diff = rest_k / n;
        value += (base_a + base_a - local_diff + 1) * local_diff / 2 * n;
        let remains = rest_k % n;
        value += remains * (base_a - local_diff);
        dbg!(local_diff, remains, value);
        println!("{}", value);
    }
...
40 60 60 70
",
            "6837" // 820 + 1830 + 1830 + 2485 - 4 * 36 + 2 * 8
        );
    }
