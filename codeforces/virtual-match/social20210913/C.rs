    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            let mut chars = vec![HashMap::new(); n];
            for i in 0..n {
                for char in s[i].chars() {
                    *chars[i].entry(char).or_insert(0isize) += 1;
                }
            }

            let mut max_val = 0isize;
            for target_char in ['a', 'b', 'c', 'd', 'e'].iter() {
                let mut values = Vec::with_capacity(n);
                for i in 0..n {
                    let num = chars[i].get(target_char).unwrap_or(&0);
                    values.push(num - (s[i].len() as isize - num));
                }
                ...
                let mut result = values.len() as isize;
                let mut sum = 0;
