    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            let mut field: Vec<Vec<Cell>> = field
                .iter()
                .map(|r| {
                    r.chars()
                        .map(|c| if c == '.' { Cell::Empty } else { Cell::Painted })
                        .collect()
                })
                .collect();
            ...
            for i in (1..n).rev() {
                for j in 0..m {
                    if field[i][j] != Cell::Empty {
                        let mut trick_size = 0;
                        for l in 1..=i {
                            if j >= l
                                && field[i - l][j - l] != Cell::Empty
                                && m > j + l
                                && field[i - l][j + l] != Cell::Empty
                            {
                                trick_size = l;
                        ...
                        if trick_size >= k {
                            field[i][j] = Cell::PaintedUsed;
                            for l in 1..=trick_size {
                                field[i - l][j - l] = Cell::PaintedUsed;
                                field[i - l][j + l] = Cell::PaintedUsed;
                            }
                        }
                    }
            ...
            if field
                .iter()
                .any(|r| r.iter().any(|cell| cell == &Cell::Painted))
            {
                println!("NO");
            } else {
                println!("YES");
