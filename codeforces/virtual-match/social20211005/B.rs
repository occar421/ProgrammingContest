    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            while a.len() > 0 {
                let min_position_rev_a = a.iter().enumerate().min_by_key(|(_, &x)| x).unwrap().0;
                let min_position = a.len() - 1 - min_position_rev_a;
                if min_position == 0 {
                    a.pop();
                    sorted_index += 1;
                    continue;
                ...
                processes.push((
                    sorted_index + 1,
                    sorted_index + 1 + min_position,
                    min_position,
                ));

                a.remove(min_position_rev_a);
                sorted_index += 1;
            }

            println!("{}", processes.len());
            for &(l, r, d) in processes.iter() {
                println!("{} {} {}", l, r, d);
            }
        }
    }
