    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            let mut unused = HashSet::from_iter(0..n);

            fn dfs(a: &Vec<usize>, unused: &mut HashSet<usize>) -> bool {
                if let Some(&x_i) = unused.iter().next() {
                    unused.remove(&x_i);
                    let x = a[x_i];

                    let mut nexts = vec![];
                    for y_i in unused.iter().filter(|y_i| {
                        let y = a[**y_i];
                        let v = max!(x, y) - min!(x, y);
                        v % 2 == 0 || v == 1
                    }) {
                        nexts.push(*y_i);
                    }
                    ...
                    for &y_i in nexts.iter() {
                        let y = a[y_i];
                        dbg!(x, y);
                        unused.remove(&y_i);
                        let result_x = dfs(a, unused);
                        unused.insert(y_i);
                        if result_x {
                            result = true;
                            break;
            ...
            println!("{}", if dfs(&a, &mut unused) { "YES" } else { "NO" });
