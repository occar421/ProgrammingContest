    ...
    W: std::io::Write,
{
...

    {
        ...
        't: for _ in 0..t {
            ...
            let mut first = s[..s.len() / 2].to_vec();
            let mut middle = (s.len() % 2 == 1).then_some_(s[s.len() / 2]);
            let mut second = s[s.len() / 2..].to_vec();
            second.reverse();
            if middle.is_some() {
                second.pop();
            }

            dbg!(first, middle, second);

            let mut n0 = a as isize;
            let mut n1 = b as isize;
            ...
            if n0 >= 0 && n0 % 2 == 0 && n1 >= 0 && n1 % 2 == 0 {
                ...
                second.reverse();
                for char in first.iter() {
                    print!("{}", char);
                }
                if let Some(m) = middle {
                    print!("{}", m);
                }
                for char in second.iter() {
                    print!("{}", char);
                }
                println!();
            } else {
