    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            input! {
                n: usize, l: usize, r: usize, s: usize,
            }

            let ns = 1 + r - l;
            let min = (ns * (ns + 1)) / 2;
            let max = (n * (n + 1)) / 2 - ((n - ns) * (n - ns + 1)) / 2;
            dbg!(min, max);
            ...
            let mut rest = s;
            let mut rest_max_num = n;
            let mut rest_min = min;
            let mut done = false;
            for cur in (1..=ns).rev() {
                if done {
                    used.insert(cur);
                ...
                rest_min -= cur;
                if rest_max_num > rest + rest_min {
                    used.insert(rest_max_num);
                    rest_max_num -= 1;
                } else if rest_max_num == rest + rest_min {
                    used.insert(rest_max_num);
                    done = true;
                } else {
                    used.insert(rest - rest_min);
                    done = true;
                }
            }
