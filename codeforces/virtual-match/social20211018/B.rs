        ...
        for _ in 0..t {
            ...
            for (i, row) in r.iter().enumerate() {
                for (j, &char) in row.iter().enumerate() {
                    if char == '*' {
                        pos.push(Point2d { x: j, y: i });
                    }
                }
            }

            let first = pos[0].clone();
            let second = pos[1].clone();
            let mut changes = vec![];
            if first.x == second.x {
                if first.x == 0 {
                    changes.push(Point2d { x: 1, y: first.y });
                    changes.push(Point2d { x: 1, y: second.y });
                } else {
                    changes.push(Point2d {
                        x: first.x - 1,
                        y: first.y,
                    });
                    changes.push(Point2d {
                ...
                if first.y == 0 {
                    changes.push(Point2d { x: first.x, y: 1 });
                    changes.push(Point2d { x: second.x, y: 1 });
                } else {
                    changes.push(Point2d {
                        x: first.x,
                        y: first.y - 1,
