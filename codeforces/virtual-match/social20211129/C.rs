    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            let mut p_deq = VecDeque::new();

            let largest = partial_max!(a).unwrap();

            let mut a_deq = VecDeque::from_iter(a);
            match largest {
                n if &n == a_deq.front().unwrap() => {
                    p_deq.push_front(n);
                    a_deq.pop_front();
                }
                n if &n == a_deq.back().unwrap() => {
                    p_deq.push_back(n);
                    a_deq.pop_back();
                }
                _ => {
            ...
            while !a_deq.is_empty() {
                let &front = a_deq.front().unwrap();
                let &back = a_deq.back().unwrap();

                if front < back {
                    p_deq.push_front(a_deq.pop_front().unwrap());
                } else {
                    p_deq.push_back(a_deq.pop_back().unwrap());
                }
            }

            while let Some(v) = p_deq.pop_front() {
                print!("{} ", v);
            }
