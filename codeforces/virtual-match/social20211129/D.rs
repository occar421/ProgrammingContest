    ...
    W: std::io::Write,
{
...

    {
        ...
        't: for _ in 0..t {
            ...
            // check
            let mut root = b.iter().enumerate().position(|(i, &b_i)| i == b_i).unwrap();
            if p[0] != root {
                println!("-1");
                continue 't;
            }
            ...
            for &curr in p.iter().skip(1) {
                let parent = b[curr];
                let curr_dist = min_v + 1;
                dist[curr] = curr_dist.into();
                if let Some(parent_dist) = dist[parent] {
                    w[curr] = curr_dist - parent_dist;
                } else {
                    println!("-1");
                    continue 't;
                }
