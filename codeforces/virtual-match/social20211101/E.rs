    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            input! {
                n: usize, m: usize,
                txy: [(usize, usize1, usize1); m],
            }

            let mut edges = HashMap::new();
            let mut degs = vec![0usize; n];
            for &(_, x, y) in txy.iter().filter(|x| x.0 == 1) {
                edges.entry(x).or_insert(vec![]).push(y);
                degs[y] += 1;
            }
            ...
            let is_dag = partial_max!(degs).unwrap_or(0) != 0;
            ...
            println!("YES");
            let mut edges = HashMap::new();
            for &(_, x, y) in txy.iter().filter(|x| x.0 == 0) {
                edges.entry(x).or_insert(HashSet::new()).insert(y);
                edges.entry(y).or_insert(HashSet::new()).insert(x);
            }
            for (i, v) in edges.iter() {
                for j in v.iter() {
                    println!("{} {}", i + 1, j + 1);
                }
            }
