    ...
    W: std::io::Write,
{
...

    {
        ...
        for _ in 0..t {
            ...
            let gen = index_to_ascii_gen('a');

            let mut vec_map: HashMap<_, Vec<usize>> = HashMap::new();
            for c in 0..26 {
                vec_map.insert(gen(c), vec![0; n]);
            }
            for (i, c) in s.chars().enumerate() {
                vec_map.get_mut(&c).unwrap()[i] += 1;
            }

            dbg!(s);

            let mut map = HashMap::new();
            for (&k, v) in vec_map.iter() {
                map.insert(k, cumulative_sum::CumulativeSum1d::new(n, v));
            }

            fn length_to_good(
                map: &HashMap<char, cumulative_sum::CumulativeSum1d<usize>>,
                range: Range<usize>,
                c: char,
            ) -> usize {
                ...
                if length <= 1 {
                    let result = 1 - map[&c].sum_in(range.clone());
                    dbg!(range, c, result);
                    return result;
                }

                let cp1 = (c as u8 + 1) as char;

                let half = range.start + length / 2;

                let n1 = length_to_good(map, range.start..half, cp1) + length / 2
                    - map[&c].sum_in(half..range.end);
                let n2 = length / 2 - map[&c].sum_in(range.start..half)
                    + length_to_good(map, half..range.end, cp1);
            ...
            println!("{}", length_to_good(&map, 0..n, 'a'));
        }
    }
