    ...
    W: std::io::Write,
{
...

    {
        input! {
            n: usize, k: usize,
            t_a_b: [(usize, usize, usize); n],
        }

        let books: Vec<_> = t_a_b
            .iter()
            .enumerate()
            .map(|(i, &(t, a, b))| (i, t, a == 1, b == 1))
            .filter(|&(_, _, a, b)| a || b)
            .collect();
        let a_books: Vec<_> = books
            .iter()
            .filter_map(|&(i, t, a, _)| a.then_some_((i, t)))
            .collect();
        let b_books: Vec<_> = books
            .iter()
            .filter_map(|&(i, t, _, b)| b.then_some_((i, t)))
            .collect();

        if a_books.len() < k || b_books.len() < k {
            println!("-1");
            return Ok(());
        }

        let mut dp = nested_vec![0; k + 1; k + 1];
