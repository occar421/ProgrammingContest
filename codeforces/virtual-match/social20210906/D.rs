    ...
    W: std::io::Write,
{
...

    {
        ...
        for i in 0..=69 {
            powers.push(2i128.pow(i).to_string());
        }

        let mut dp = nested_vec![0usize; 25; 25];

        for &n in n.iter() {
            let mut min_match = usize::max_value();

            let n = n.to_string();
            for power in powers.iter() {
                let n = n.as_bytes();
                let p = power.as_bytes();
                let match_length = n.iter().zip(p.iter()).filter(|t| t.0 == t.1).count();

                let n = &n[match_length..];
                let p = &p[match_length..];
                if n.len() == 0 || p.len() == 0 {
                    min_match = min!(min_match, max!(n.len(), p.len()));
                    continue;
                }

                dbg!(match_length, n, p);

                // n\p| a | b |
                //  a | 1 |   |
                //  d |   |   |
                for (i, p_i) in n.iter().enumerate() {
                    for (j, p_j) in p.iter().enumerate() {
                        dp[i][j] = if p_i == p_j { 1 } else { 0 };
                    } // O(N)
                } // O(N^2)
                ...
                min_match = min!(min_match, dp[n.len() - 1][p.len() - 1]);
