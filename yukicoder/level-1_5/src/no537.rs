...
pub trait GenericInteger: Copy + PartialEq + FromStr + Display + Rem<Output=Self> + Div<Output=Self> + Mul<Output=Self> {
    fn zero() -> Self;
}
...
fn process<R, W>(mut reader: R, mut writer: W) -> Result<()> where
    ...
    let rt = (n as f32).sqrt().floor() as u64;
    let set: HashSet<_> = HashSet::from_iter((1..=rt).filter(|x| n % x == 0)
        .flat_map(|x| vec![vec![x, n / x].add_like_string(), vec![n / x, x].add_like_string()])
    );

    write!(writer, "{}", set.len())?;
...
mod tests {
    ...
    #[test]
    fn sample2() {
        assert_judge!(process, "63", "6");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "49", "3");
    }

    #[test]
    fn small4() {
        assert_judge!(process, "64", "6");
    }
}