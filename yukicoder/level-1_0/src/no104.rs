...
fn process<R, W>(mut reader: R, mut writer: W) -> Result<()> where
    ...
    let route_number = s.iter().fold(1, |acc, d|
        if d == &'L' { acc * 2 } else { acc * 2 + 1 },
    );

    write!(writer, "{}", route_number)?;
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "LR", "5");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "RLL", "12");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "", "1");
    }
}