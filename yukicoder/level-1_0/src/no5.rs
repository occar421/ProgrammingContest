...
fn process<R, W>(mut reader: R, mut writer: W) -> Result<()> where
    ...
    let (_, result) = w.iter().fold((0, 0), |(occupied_space, count), &wi|
        if occupied_space + wi > l { (occupied_space, count) } else { (occupied_space + wi, count + 1) },
    );
...
100
10
14 85 77 26 50 45 66 79 10 3", "5");
    }
}