...
fn process<R, W>(mut reader: R, mut writer: W) -> () where
    ...
    let x: HashSet<&i64> = HashSet::from_iter(x.iter());
    let mut dp = vec![0; 10usize.pow(6) + 1];
    for i in (1..dp.len()).rev() {
        dp[i] = if x.contains(&(i as i64)) {
            (i..dp.len()).step_by(i).map(|j| dp[j] + 1).max().unwrap()
        } else { std::i64::MIN };
    }

    write!(writer, "{}", dp.iter().max().unwrap());
}
