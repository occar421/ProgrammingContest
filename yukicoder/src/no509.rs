...
fn process<R, W>(mut reader: R, mut writer: W) -> () where
    ...
    let n_holes_map = [1, 0, 0, 0, 1, 0, 1, 0, 2, 1];

    let n_paint_holes: usize = n.iter().map(|n| n_holes_map[*n as usize - '0' as usize]).sum();

    // paint number n times (red) + paint bg (black) + paint holes (black) + paint number n times (white)
    let r1 = n.len() * 2 + 1 + n_paint_holes;

    // paint bg (red) + paint holes (red) + paint number n times (white) + paint bg (black) + paint holes (black)
    let r2 = 1 * 2 + n_paint_holes * 2 + n.len();

    write!(writer, "{}", min(r1, r2));
}
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "6", "4");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "1", "3");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "1046928", "21");
    }

    #[test]
    fn gen_case1() {
        assert_judge!(process, "105104931", "19");
    }
}