...
fn process<R, W>(mut reader: R, mut writer: W) -> () where
    ...
    let mut e = vec![1.0; k + 6];
    for x in k..k + 6 {
        e[x] = 0.0;
    }

    for x in (0..k).rev() {
        e[x] = e[x + 1] / 6.0 + e[x + 2] / 6.0 + e[x + 3] / 6.0 + e[x + 4] / 6.0 + e[x + 5] / 6.0 + e[x + 6] / 6.0 + 1.0;
    }
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge_with_error!(process, "1", "1", f64 | 10f64.powi(-2));
    }

    #[test]
    fn sample2() {
        assert_judge_with_error!(process, "2", "1.16667", f64 | 10f64.powi(-2));
    }

    #[test]
    fn sample3() {
        assert_judge_with_error!(process, "3", "1.36111", f64 | 10f64.powi(-2));
    }
