    ...
    W: std::io::Write,
{
...

    {
        ...
        for i in 0..h {
            for j in 0..w {
                edges.insert((i, j), Vec::<((usize, usize), usize)>::with_capacity(4));
                nodes.insert((i, j));
            }
        }
        for i in 0..h {
            for j in 0..w - 1 {
                edges
                    .get_mut(&(i, j))
                    .unwrap()
                    .push(((i, j + 1), if s[i][j + 1] == '#' { 1 } else { 0 }));
                edges
                    .get_mut(&(i, j + 1))
                    .unwrap()
                    .push(((i, j), if s[i][j] == '#' { 1 } else { 0 }));
            }
        }
        for i in 0..h - 1 {
            for j in 0..w {
                edges
                    .get_mut(&(i, j))
                    .unwrap()
                    .push(((i + 1, j), if s[i + 1][j] == '#' { 1 } else { 0 }));
                edges
                    .get_mut(&(i + 1, j))
                    .unwrap()
                    .push(((i, j), if s[i][j] == '#' { 1 } else { 0 }));
        ...
        let mut counter = 0;
        loop {
            let mut g = graph::Graph::new(&nodes, &edges);

            let memo = g.dijkstra((0, 0), 0);

            let c = memo.cost_to((h - 1, w - 1)).unwrap();

            if c == 0 {
                break;
            }

            let path = memo.path_to((h - 1, w - 1)).unwrap();
            // edges[path[0]]
        }
