    ...
    W: std::io::Write,
{
...

    {
        ...
        for &(u, v, w) in uvw.iter() {
            edges.entry(u).or_insert(vec![]).push((v, w));
            memo.insert((u, v), w);
            edges.entry(v).or_insert(vec![]).push((u, w));
            memo.insert((v, u), w);
        }
        ...
        fn dfs(
            ...
            dbg!("ent", current, target);
            if let Some(&memoized) = memo.get(&(current, target)) {
                dbg!(memoized);
                dbg!();
                return memoized.into();
            }

            for &(next, cost) in edges[&current].iter() {
                ...
                dbg!("loop", current, target);
                let result = dfs(next, target, current.into(), edges, memo);
                if let Some(result) = result {
                    let value = max!(cost, result);
                    memo.insert((current, target), value);
                    memo.insert((target, current), value);
                    dbg!(value);
                    dbg!();
                }
            }

            return memo.get(&(current, target)).map(|x| *x);
        }

        let mut s = 0;
        for i in 0..n - 1 {
            for j in i + 1..n {
                s += dfs(i, j, None, &edges, &mut memo).unwrap();
            }
        }
