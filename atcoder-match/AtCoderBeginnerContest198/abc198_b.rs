    ...
    W: Write,
{
    ...
    let stem_n = n.trim_end_matches('0').to_string();

    let reversed_stem_n: String = stem_n.chars().rev().collect();

    writeln!(
        writer,
        "{}",
        if stem_n == reversed_stem_n {
            "Yes"
        } else {
            "No"
        }
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "1210", "Yes");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "777", "Yes");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "123456789", "No");
    }
}
