...

    {
        ...
        for &(a, c) in ac.iter() {
            if prev != c {
                ...
                ac_map.last_mut().unwrap().push((a, c));
            }
        }

        let mut cost = 0;
        let mut remains = n;
        for ac_s in ac_map.iter() {
            let mut aaa = ac_s;
            loop {
                let a = aaa
                    .iter()
                    .map(|&(a, _)| if remains % a != 0 { 0 } else { a })
                    .enumerate();
            }
            for &(a, c) in aaa.iter() {
                let a = a % remains;
                let a = a.min(remains - a);
                dbg!(a, c);
                dbg!(remains, cost);
                if a == 0 {
                    continue;
