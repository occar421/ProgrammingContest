...

    {
        ...
        for i in 1..n {
            for j in 0..n {
                if s[i - 1][j] == '.' && s[i][j] == '.' {
                    for kk in 0..=k {
                        edges.get_mut(&(i - 1, j)).unwrap().push((i, j));
                        edges.get_mut(&(i, j)).unwrap().push((i - 1, j));
                    }
                }
            }
        }
        ...
        for i in 0..n {
            for j in 1..n {
                if s[i][j - 1] == '.' && s[i][j] == '.' {
                    for kk in 0..=k {
                        edges.get_mut(&(i, j - 1)).unwrap().push((i, j));
                        edges.get_mut(&(i, j)).unwrap().push((i, j - 1));
                    }
                }
            }
        }

        // // [i][j][is_red][red_count] = count
        // // let mut dp = nested_vec![0; n; n; 2; k+1];
        // let mut count = 0;
        // let mut queue = VecDeque::new();
        // let hs: HashSet<(usize, usize)> = HashSet::new();
        // queue.push_back((0, 0, false, hs)); // i, j, is_red, path
        // while let Some((i, j, is_red, path_set)) = queue.pop_front() {
        //     if s[i][j] == '#' || path_set.contains(&(i, j)) {
        //         continue;
        //     }
        //
        //     if is_red && path_set.len() == k - 1 {
        //         count += 1;
        //         continue;
        //     }
        //
        //     if is_red {
        //         let path_set = path_set.clone().insert((i, j));
        //         for edge in edges[&(i, j)].iter() {
        //             queue.push_back((i, j, false))
        //         }
        //     } else {
        //         let path_set = path_set.clone().insert((i, j));
        //         queue.push_back((i, j, true, path_set));
        //     }
        // }
        //
        // println!();

        let mut all_white = Vec::new();
        for i in 0..n {
            for j in 0..n {
                if s[i][j] == '.' {
                    all_white.push((i, j));
        ...
        fn a(
            ...
            rest: usize,
            path: Vec<usize>,
            all_white: &Vec<(usize, usize)>,
            edges: &HashMap<(usize, usize), Vec<(usize, usize)>>,
        ) -> usize {
            if rest == 0 {
                let points: Vec<_> = path.iter().map(|&i| all_white[i]).collect();
            ...
            let mut count = 0;
            for i in start..(end - rest) {
                // take i
                let mut path = path.clone();
                path.push(i);
                count += a(i + 1, end, rest - 1, path, &all_white, &edges);
            }
            return count;
        }

        let result = a(0, n, k, vec![], &all_white, &edges);
