    ...
    W: std::io::Write,
{
...

    {
        ...
        for &next in edges[&0].iter() {
            queue.push_back((next, 0, 1)); // id, from, length
        }

        let mut visited_nodes = vec![None; n]; // (count, len)
        visited_nodes[0] = Some((ModMil7::new(1), 0));
        while let Some((current, from, len)) = queue.pop_front() {
            dbg!(current, len, visited_nodes);
            if let Some((count, memo_len)) = visited_nodes[current] {
                if len == memo_len {
                    visited_nodes[current] = Some((count + visited_nodes[from].unwrap().0, len));
                }
                continue;
            } else {
                visited_nodes[current] = Some((visited_nodes[from].unwrap().0, len));
            }

            for &next in edges[&current].iter() {
                queue.push_back((next, current, len + 1));
            }
        }

        println!(
            "{}",
            visited_nodes[n - 1].map(|x| x.0).unwrap_or(ModMil7::new(0))
        );
    }
