    ...
    W: std::io::Write,
{
...

    {
        ...
        let mut values = vec![ModMil7::new(0usize); 9];
        values[0] = ModMil7::new(1);

        for char in s.chars() {
            let mut index = match char {
                'c' => 1,
                'h' => 2,
                'o' => 3,
                'k' => 4,
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "chchokudai", "3");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "atcoderrr", "0");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "chokudaichokudaichokudai", "45");
    }
}
