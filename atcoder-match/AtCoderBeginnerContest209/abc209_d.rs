    ...
    W: Write,
{
...

    {
        ...
        input! {
            n: Length, q: Length,
            ab: [(NodeIndex1Based, NodeIndex1Based); n - 1],
            cd: [(NodeIndex1Based, NodeIndex1Based); q],
        }

        // To be in O(NlogN)

        let mut dist = nested_vec![None; n; n];
        ...
        for &(c, d) in cd.iter() {
            let c = c - 1;
            let d = d - 1;
            if let Some(cost) = dist[c][d] {
                results.push(cost);
                continue;
            }

            let mut reached_nodes = HashMap::new();
            let mut heap = BinaryHeap::new();
            heap.push(Reverse((0, c, None)));
            while let Some(Reverse((cost, current, prev))) = heap.pop() {
                if reached_nodes.contains_key(&current) {
                    continue;
                }
                reached_nodes.insert(current, (cost, prev));

                if current == d {
                    break;
                }

                for &next_node in edges[&current].iter() {
                    heap.push(Reverse((cost + 1, next_node, current.into())))
                }
            }
        }

        for result in results {
            println!("{}", if result % 2 == 0 { "Town" } else { "Road" });
        }
    }
