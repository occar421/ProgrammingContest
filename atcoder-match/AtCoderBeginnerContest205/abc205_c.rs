    ...
    W: Write,
{
...

    {
        ...
        let result = match ((a >= 0) == (b >= 0), c.cmp(&0), c % 2 == 0) {
            (_, Ordering::Equal, _) => Ordering::Equal,
            (true, Ordering::Greater, _) => a.cmp(&b),
            (true, Ordering::Less, _) => b.cmp(&a),
            (false, Ordering::Greater, true) => a.abs().cmp(&b.abs()),
            (false, Ordering::Greater, false) => a.cmp(&b),
            (false, Ordering::Less, true) => b.abs().cmp(&a.abs()),
            (false, Ordering::Less, false) => b.cmp(&a),
        };
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "3 2 4", ">");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "-7 7 2", "=");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "-8 6 3", "<");
    }
}
