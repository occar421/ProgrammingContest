    ...
    W: Write,
{
    ...
    {
        input! {
            n: Quantity, q: Quantity,
            a: [usize; n],
            k: [usize; q],
        }

        let mut segments: Vec<(usize, usize)> = vec![(0, 0)];
        for &a_i in a.iter() {
            if let Some(last) = segments.last() {
                if last.0 + last.1 == a_i {
        ...
        for i in 0..segments.len() {
            let n_v = acc;
            acc += segments[i].1;
            segments[i] = (segments[i].0 - n_v, acc);
        } // O(N)
          // seg[i] = (q個目, q までで除かれた個数)

        for &k_i in k.iter() {
            let mut ng = segments.len();
            let mut known_less = 0;
            while known_less + 1 < ng {
                let mid = (known_less + ng) / 2;
                if segments[mid].0 > k_i {
                    ng = mid;
                } else if segments[mid].0 == k_i {
                    known_less = mid;
                    break;
                } else {
                    known_less = mid;
            ...
            let result = segments[known_less.min(segments.len() - 1)];

            println!("{}", k_i + result.1);
        } // O( Q log(N) )
    } // O( N + Q log(N) )
