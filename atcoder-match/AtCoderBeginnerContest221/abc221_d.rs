    ...
    W: std::io::Write,
{
...

    {
        ...
        for &(start, length) in ab.iter() {
            *users.entry(start).or_insert(0) += 1isize;
            *users.entry(start + length).or_insert(0) -= 1;
        }

        let mut a = vec![];
        let mut cum = 0;
        for (&n, &sign) in users.iter() {
            a.push((n, sign + cum));
            cum += sign;
        }
        ...
        for w in a.windows(2) {
            ...
            *result_map.entry(current.1).or_insert(0) += next.0 - current.0;
        }

        for i in 1..=n {
            print!("{} ", result_map.get(&(i as isize)).unwrap_or(&0));
        }
    }
