    ...
    W: std::io::Write,
{
...

    {
        ...
        fn aaa(sx: Vec<char>) -> Vec<String> {
            if sx.len() == 0 {
                return vec!["".into()];
            }
            let mut a = vec![];
            for (i, sxs) in sx.iter().enumerate() {
                let mut sx = sx.clone();
                sx.remove(i);
                let results = aaa(sx);
                for result in results {
                    a.push(format!("{}{}", sxs, result));
                }
            }
            return a;
        }
        ...
        let mut a: Vec<_> = strings.into_iter().collect();
...
mod tests {
    ...
    #[test]
    fn sample1() {
        assert_judge!(process, "aab 2", "aba");
    }

    #[test]
    fn sample2() {
        assert_judge!(process, "baba 4", "baab");
    }

    #[test]
    fn sample3() {
        assert_judge!(process, "ydxwacbz 40320", "zyxwdcba");
    }
}
