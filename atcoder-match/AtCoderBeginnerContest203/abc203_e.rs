    ...
    W: Write,
{
...

    {
        ...
        // for &(x, y) in xy.iter() {
        //     xs.entry(x).or_insert(vec![]).push(y);
        // }
        //
        // let mut results = vec![];
        // let mut current_x = 0;
        // for i in (1..n).rev() {
        //     dbg!(i, current_x);
        //     if let Some(vec) = xs.get(&i) {
        //         if let Some(&min) = vec.iter().filter(|&&x| current_x < x).min() {
        //             results.push(i);
        //             current_x = min;
        //         } else {
        //             break;
        ...
        // for i in (n + 1)..(2 * n) {
        //     if let Some(vec) = xs.get(&i) {
        //         if let Some(&min) = vec.iter().filter(|&&x| x < current_x).min() {
        //             results.push(i);
        //             current_x = min;
        //         }
        //     } else {
        ...
        let blacks = HashSet::<_>::from_iter(xy.into_iter());

        fn dfs(
            edge: Length,
            blacks: &HashSet<(usize, usize)>,
            current: (usize, usize),
        ) -> HashSet<usize> {
            ...
            if !blacks.contains(&(current.0 + 1, current.1)) {
                result = &result | &dfs(edge, blacks, (current.0 + 1, current.1));
            }
            if current.1 < edge && blacks.contains(&(current.0 + 1, current.1 + 1)) {
                result = &result | &dfs(edge, blacks, (current.0 + 1, current.1 + 1));
            }
            if 1 <= current.1 && blacks.contains(&(current.0 + 1, current.1 - 1)) {
                result = &result | &dfs(edge, blacks, (current.0 + 1, current.1 - 1));
            }
            return result;
        }

        let result = dfs(2 * n, &blacks, (0, n));
