    ...
    W: std::io::Write,
{
...

    {
        ...
        for i in 0..n - 1 {
            let diff = a[i] - a[i + 1];
            dbg!(i, a[i], a[i + 1], diff);
            if diff * i >= rest_k {
                let local_diff = rest_k / (i + 1);
                value += (a[i] + a[i] - local_diff + 1) * local_diff / 2 * (i + 1);
                dbg!(value);
                let remains = rest_k % (i + 1);
            ...
            value += (a[i] + a[i] - diff + 1) * diff / 2 * (i + 1);
            rest_k -= diff * (i + 1);
            dbg!(value, rest_k);
        }
        ...
        dbg!(local_diff);
        dbg!(a[n - 1]);
        value += (a[n - 1] + a[n - 1] - local_diff + 1) * local_diff / 2 * n;
        dbg!(value);
        let remains = rest_k % n;
        value += remains * (a[n - 1] - local_diff);
        dbg!(value);
        println!("{}", value);
    }
