    ...
    W: std::io::Write,
{
...

    {
        ...
        let mut twos = VecDeque::new();
        let mut pos = HashMap::new();
        for i in 0..n {
            pos.insert(i, Vec::<usize>::with_capacity(2));
        }
        ...
        for i in 0..m {
            ...
            for a_k_i in a_k.iter() {
                pos.get_mut(&a_k_i).unwrap().push(i);
            }
            let mut a_k = VecDeque::from_iter(a_k);
            let top = a_k.pop_front().unwrap();
            if ones.take(&top).is_some() {
                twos.push_back(top);
            } else {
                ones.insert(top);
            }
        ...
        while let Some(ball) = twos.pop_front() {
            let pos = pos.get(&ball).unwrap();
            for &p in pos.iter() {
                if let Some(a) = a.get_mut(p).unwrap().pop_front() {
                    if ones.take(&a).is_some() {
                        twos.push_back(a);
                    } else {
                        ones.insert(a);
                    }
        ...
        println!("{}", if ones.is_empty() { "Yes" } else { "No" });
    }
